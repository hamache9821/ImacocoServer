...
function updateUserLabel()
{
	...
	if (!labelname)
	{
		alert("id='labelname' not found in form element.");
		return false;
	}
	...
	if (!user_id)
	{
		alert("id='user_ust' not found in form element.");
		return false;
	}

	if (!checkInvalidCharNickname(labelname.value))
	{
		alert("<>\"\\は使用できません");
		return false;
	}
	var url = "http://www.imacoconow.net/cgi-bin/make_label2.pl" +
		"?nickname=" + encodeURIComponent(labelname.value) + 
		"&id=" + encodeURIComponent(user_id.value);
	window.open(url, '表示名変更', 'width=180, height=100');
	return false;
}
